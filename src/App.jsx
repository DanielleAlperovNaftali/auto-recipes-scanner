...
function Modal({ children, onClose, wide }) {
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.72)", backdropFilter:"blur(5px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:"1rem" }}>
      <div onClick={e => e.stopPropagation()} style={{ background:"linear-gradient(135deg,#1a1740,#2a2460)", borderRadius:"24px", border:"1px solid rgba(255,255,255,0.12)", padding:"2rem", maxWidth: wide?"800px":"540px", width:"100%", maxHeight:"90vh", overflowY:"auto", boxShadow:"0 24px 80px rgba(0,0,0,0.6)" }}>
        {children}
      </div>
    </div>
  );
...
function RecipeApp({ session, onLogout }) {
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0f0c29,#302b63,#24243e)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"1rem", color:"#f8b500" }}>
      <div style={{ fontSize:"3rem" }}>🍽️</div>
      <p>טוען מתכונים...</p>
    </div>
  );
    ...
    <div style={S.app}>
      ...
      <div style={{ ...S.main, display:"flex", alignItems:"center", justifyContent:"center" }}>
        <div style={{ textAlign:"center" }}>
          {scanning ? <><div style={{ fontSize:"3rem", marginBottom:"1rem" }}>🔍</div><p style={{ fontSize:"1.3rem", color:"#f8b500" }}>מעבד את המתכון...</p><p style={{ color:"#aaa" }}>מזהה טקסט עברי ומסווג בקטגוריות</p></>
      ...
      <div style={S.app}>
        ...
        {editTitleFor && (
          <Modal onClose={() => setEditTitleFor(null)}>
            ...
            <input value={editTitleVal} onChange={e => setEditTitleVal(e.target.value)} dir="rtl"
        ...
        <header style={S.hdr}>
          ...
          <div style={{ display:"flex", gap:"0.5rem", flexWrap:"wrap", alignItems:"center" }}>
            ...
            {!selectedRecipe.isDemo && <>
              ...
              <button style={{ ...S.nb(false), color:"#60A5FA", borderColor:"#60A5FA44" }} onClick={() => { setEditTitleFor(selectedRecipe.id); setEditTitleVal(selectedRecipe.title); }}>✏️</button>
  ...
  return (
    <div style={S.app}>
      ...
      <header style={S.hdr}><span style={S.logo}>🍽️ מתכונים שלי</span><NavButtons /><FileInput /></header>
      <div style={S.main}>
